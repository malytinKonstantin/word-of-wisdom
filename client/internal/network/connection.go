...
func NewDefaultNetworkClient(cfg *config.Config) *DefaultNetworkClient {
	...
	if err != nil {
		log.Fatal().Err(err).Msg("Не удалось прочитать сертификат сервера")
	}
...
func (nc *DefaultNetworkClient) Connect(serverAddr string) (net.Conn, error) {
	log.Info().Str("server_address", serverAddr).Msg("Попытка подключения к серверу")
	conn, err := tls.Dial("tcp", serverAddr, nc.tlsConfig)
	if err != nil {
		return nil, fmt.Errorf("ошибка подключения к серверу: %v", err)
	}
	log.Info().Str("remote_address", conn.RemoteAddr().String()).Msg("TLS соединение установлено")
	return conn, nil
}
...
func (nc *DefaultNetworkClient) ReceiveChallenge(conn net.Conn) (string, int, error) {
	...
	log.Info().Msg("Ожидание challenge от сервера")
	challenge, err := readLine(reader)
	if err != nil {
		return "", 0, fmt.Errorf("ошибка получения challenge: %v", err)
	}
	log.Info().Str("challenge", challenge).Msg("Получен challenge")

	log.Info().Msg("Ожидание сложности от сервера")
